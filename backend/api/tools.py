...
async def approve_tool(
    ...
    if scope == PermissionScope.NEVER:
        await audit.log_tool_rejection(
            request.session_id, request.tool, request.params, "blocked"
        )
    else:
        await audit.log_tool_approval(
            request.session_id, request.tool, request.params, scope.value
        )

    await db.commit()

    return {"status": "ok", "tool": request.tool, "decision": request.decision}
