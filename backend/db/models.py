...
class Conversation(Base):
    ...
    audit_logs = relationship(
        "AuditLog", back_populates="conversation", cascade="all, delete-orphan"
    )
...
class AuditLog(Base):
    ...
    )  # tool_requested, tool_approved, tool_rejected, tool_executed, tool_failed
...
class Memory(Base):
    ...
    )  # Vektor-Embedding als Bytes (numpy float32)
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class Agent(Base):
    ...
    )  # z.B. "ollama/qwen2.5:7b", "claude-sonnet", None = global default
    allowed_tools = Column(
        JSON, nullable=True
    )  # ["web_search", "file_read"] oder None = alle
    allowed_skills = Column(JSON, nullable=True)  # ["email_inbox"] oder None = alle
    risk_level_max = Column(
        String(20), default="high"
    )  # low, medium, high — max ohne Approval
    auto_approve_tools = Column(
        JSON, nullable=True
    )  # ["web_search"] — Tools ohne Approval
...
class Workflow(Base):
    ...
    name = Column(String(255), nullable=False)
    description = Column(Text, nullable=True)
    trigger_phrase = Column(
        String(255), nullable=True
    )  # z.B. "Tagesstart", "Wochenbericht"
    agent_id = Column(String(36), ForeignKey("agents.id"), nullable=True)
    steps = Column(JSON, nullable=False)  # [{order, prompt, store_as}]
    approval_mode = Column(
        String(20), default="each_step"
    )  # each_step, once_at_start, never
    enabled = Column(Boolean, default=True)
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
